...
// global buffer
//
DEBUG_USB_GLOBAL_DATA* BdUsbpGlobalData										= NULL;
...
STATIC BOOLEAN BdUsbpWait(DEBUG_USB_GLOBAL_DATA* globalData, UINT32 millisecond)
{
	...
	__try
	{
		...
		while(TRUE)
		{
			...
			// XXX: overflow?
			//
			if((UINT32)((temp & 0x3fff) - startIndex) > millisecond * 8)
				break;
		}
		retValue															= TRUE;
#if defined(_MSC_VER)
...
STATIC BOOLEAN BdUsbpTrySetupDevice(DEBUG_USB_GLOBAL_DATA* globalData)
{
	...
	for(UINT32 retry = 0; retry < 3; retry ++)
	{
		...
		if(!BdUsbpControlWrite(globalData, 0, &setupPacket, NULL, 0))
			return FALSE;
		...
		setupPacket.Index													= 0;
		setupPacket.Length													= 0;
		return BdUsbpControlWrite(globalData, globalData->UsbAddress, &setupPacket, NULL, 0);
	}
	return FALSE;
}
...
VOID BdUsbSendPacket(UINT32 packetType, STRING* messageHeader, STRING* messageData)
{
	...
	while(TRUE)
	{
		...
		// receive ACK
		//
		UINT32 result														= BdUsbReceivePacket(PACKET_TYPE_KD_ACKNOWLEDGE, NULL, NULL, NULL);
...
EFI_STATUS BdUsbConfigureDebuggerDevice(CHAR8 CONST* loaderOptions)
{
	...
	__try
	{
		...
		if(connectWait)
		{
			UINTN waitTime													= (UINT32)(atoi(connectWait + 13));
        ...
        if(EFI_ERROR(status	= BlFindPciDevice(segment, bus, device, func, PCI_CLASS_SERIAL, PCI_CLASS_SERIAL_USB, 0x20, &pciIoProtocol, NULL))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if(EFI_ERROR(status) && BdUsbpGlobalData)
		...
		//
		BdUsbpGlobalData													= (DEBUG_USB_GLOBAL_DATA*)(MmAllocatePool(sizeof(DEBUG_USB_GLOBAL_DATA)));
        if(!BdUsbpGlobalData) {
#if defined(_MSC_VER)
