...
EFI_STATUS BlConnectAllController()
{
	...
	__try
	{
		...
		while (TRUE)
		{
            ...
            if (EFI_ERROR(status = EfiBootServices->LocateHandleBuffer(AllHandles, NULL, NULL, &totalHandles, &handleArray))) {
#if defined(_MSC_VER)
                try_leave(NOTHING);
#else
                if (handleArray)
			...
			for (UINTN i = 0; i < totalHandles; i ++)
				EfiBootServices->ConnectController(handleArray[i], NULL, NULL, TRUE);
...
{
	EFI_LOADED_IMAGE_PROTOCOL* loadeImageProtocol							= NULL;

	if (EfiBootServices->HandleProtocol(EfiImageHandle, &EfiLoadedImageProtocolGuid, (VOID**)(&loadeImageProtocol)) != EFI_SUCCESS)
...
{
	VOID* pciResource														= NULL;
	EFI_STATUS Status														= pciIoProtocol->GetBarAttributes(pciIoProtocol, barIndex, NULL, &pciResource);
	...
	if (!EFI_ERROR(Status) && pciResource)
	{
		...
		//
		EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR* desc								= (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR*)(pciResource);
...
EFI_STATUS BlFindPciDevice(UINTN segment, UINTN bus, UINTN device, UINTN func, UINT8 baseClass, UINT8 subClass, UINT8 progIf, EFI_PCI_IO_PROTOCOL** outPciIoProtocol, EFI_HANDLE* outHandle)
{
	...
	__try
	{
		...
		//
        if (EFI_ERROR(status = EfiBootServices->LocateHandleBuffer(ByProtocol, &EfiPciIoProtocolGuid, NULL, &totalHandles, &handleArray))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if (handleArray)
		...
		for (UINTN i = 0; i < totalHandles; i ++)
		{
			...
			EFI_PCI_IO_PROTOCOL* pciIoProtocol								= NULL;

			//
			// If open failed, skip it.
			//
			if (EFI_ERROR(EfiBootServices->HandleProtocol(theHandle, &EfiPciIoProtocolGuid, (VOID**)(&pciIoProtocol))))
				continue;

			//
			// Get location.
			//
			UINTN curSeg													= (UINTN)(-1);
			UINTN curBus													= (UINTN)(-1);
			UINTN curDev													= (UINTN)(-1);
			UINTN curFunc													= (UINTN)(-1);
...
#else
                if (outPciIoProtocol)
                    *outPciIoProtocol = pciIoProtocol;
...
EFI_STATUS BlStartPciDevice(EFI_PCI_IO_PROTOCOL* pciIoProtocol, BOOLEAN decodeIo, BOOLEAN decodeMemory, BOOLEAN busMaster)
{
	...
	return pciIoProtocol->Attributes(pciIoProtocol, EfiPciIoAttributeOperationEnable, attribute, NULL);
}
...
UINT8* BlpGetStringFromSMBIOSTable(UINT8* startOfStringTable, UINT8 index)
{
	for (UINT8 i = 1; i < index && *startOfStringTable; i ++)
		startOfStringTable													+= strlen((CHAR8*)(startOfStringTable)) + 1;
...
EFI_STATUS BlDetectMemorySize()
{
	...
	for (UINTN i = 0; i < EfiSystemTable->NumberOfTableEntries; i ++, theTable ++)
	{
		...
		SMBIOS_ENTRY_POINT_STRUCTURE* tableStructure						= (SMBIOS_ENTRY_POINT_STRUCTURE*)(theTable->VendorTable);
		...
		{
			SMBIOS_TABLE_HEADER* tableHeader								= (SMBIOS_TABLE_HEADER*)(startOfTable);
			...
			if (tableHeader->Type == 16) // Physical Memory Array.
			{
				...
				SMBIOS_TABLE_TYPE16* table16								= (SMBIOS_TABLE_TYPE16*)(startOfTable);

				// Capacity present?
				if (table16->MaximumCapacity != 0x80000000)
				{
					// Maximum memory capacity in kilobytes.
					BlpMemoryCapacity										+= ((UINT64)(table16->MaximumCapacity) << 10);
			...
			else if (tableHeader->Type == 17) // Memory Device.
			{
				...
				SMBIOS_TABLE_TYPE17* table17								= (SMBIOS_TABLE_TYPE17*)(startOfTable);
			...
			else if (tableHeader->Type == 1) // System Information.
			{
				...
				SMBIOS_TABLE_TYPE1* table1									= (SMBIOS_TABLE_TYPE1*)(startOfTable);
			...
			else if (tableHeader->Type == 2) // Baseboard (or Module) Information.
			{
				...
				SMBIOS_TABLE_TYPE2* table2									= (SMBIOS_TABLE_TYPE2*)(startOfTable);

				if (table2->ProductName)
				{
					UINT8* boardId											= BlpGetStringFromSMBIOSTable(startOfTable + table2->Hdr.Length, table2->ProductName);
					strncpy(BlpBoardId, (CHAR8*)(boardId), ARRAYSIZE(BlpBoardId) - 1);
				}
			}
...
EFI_STATUS BlUtf8ToUnicode(CHAR8 CONST* utf8Buffer, UINTN bytesCount, CHAR16* unicodeBuffer, UINTN unicodeBufferLengthInChar)
{
	...
	while (i < bytesCount)
	{
		...
		else if (utf8Char1 < 0xe0)
		{
			...
			UINT16 unicodeChar												= ((UINT16)(utf8Char1 & 0x0f) << 6) | (utf8Char2 & 0x3f);
...
CHAR8* BlAllocateUtf8FromUnicode(CHAR16 CONST* unicodeString, UINTN unicodeCharCount)
{
	...
	UINT8* utf8NameBuffer													= (UINT8*)(MmAllocatePool(utf8BufferLength));
...
CHAR16* BlAllocateUnicodeFromUtf8(CHAR8 CONST* utf8String, UINTN utf8Length)
{
	...
	CHAR16* unicodeBuffer													= (CHAR16*)(MmAllocatePool(length * sizeof(CHAR16)));
...
{
	CHAR8* retValue															= (UINT8*)(MmAllocatePool(strlen(inputString) + 1));
...
UINT32 BlAdler32(VOID CONST* inputBuffer, UINTN bufferLength)
{
	UINT32 lowHalf															= 1;
	UINT32 highHalf															= 0;
	UINT8 CONST* theBuffer													= (UINT8 CONST*)(inputBuffer);
...
EFI_STATUS BlDecompressLZSS(VOID CONST* compressedBuffer, UINTN compressedSize, VOID* uncompressedBuffer, UINTN uncompressedBufferSize, UINTN* uncompressedSize)
{
	...
	UINT8* dstBuffer														= (UINT8*)(uncompressedBuffer);
	UINT8* dstStart															= dstBuffer;
	UINT8 CONST* srcBuffer													= (UINT8 CONST*)(compressedBuffer);
