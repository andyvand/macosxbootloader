...
VOID CONST* AcpipGetTable(UINT32 signature)
{
	...
	EFI_ACPI_2_0_ROOT_SYSTEM_DESCRIPTION_POINTER CONST* acpiRsdp			= NULL;
	for(; i < EfiSystemTable->NumberOfTableEntries; i++, theTable++)
	{
		...
		{
			acpiRsdp														= (EFI_ACPI_2_0_ROOT_SYSTEM_DESCRIPTION_POINTER CONST*)(theTable->VendorTable);
			break;
		}
	...
	EFI_ACPI_DESCRIPTION_HEADER* acpiRsdt									= NULL;
	UINT32 tableCount														= 0;
	UINT32 itemSize															= 0;
	...
	{
		UINT64 tableAddress													= itemSize == sizeof(UINT64) ? *(UINT64*)(tableArray) : *(UINT32*)(tableArray);
...
{
	EFI_ACPI_DESCRIPTION_HEADER CONST* header								= (EFI_ACPI_DESCRIPTION_HEADER CONST*)(AcpipGetTable(0x4746434D));
