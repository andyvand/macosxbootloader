...
EFI_STATUS PeInitialize()
{
	...
	EFI_DATA_HUB_PROTOCOL* dataHubProtocol									= NULL;
	EFI_STATUS status														= EfiBootServices->LocateProtocol(&EfiDataHubProtocolGuid, NULL, (VOID**)(&dataHubProtocol));
	...
	do
	{
		EFI_DATA_RECORD_HEADER* recordHeader								= NULL;
		...
		{
			APPLE_SYSTEM_INFO_DATA_RECORD* dataRecord						= (APPLE_SYSTEM_INFO_DATA_RECORD*)((VOID*)(recordHeader + 1));
			UINTN nameLength												= (dataRecord->NameLength / sizeof(CHAR16) * 3 + 1) * sizeof(CHAR8);
			CHAR8* utf8NameBuffer											= (CHAR8*)(MmAllocatePool(nameLength));
...
CHAR8 CONST* PeGetModelName()
{
	...
	__try
	{
		...
		{
			UINT32 nameLength												= 0;
			CHAR16 CONST* unicodeModelName									= (CHAR16 CONST*)(DevTreeGetProperty(platformNode, CHAR8_CONST_STRING("Model"), &nameLength));
			...
			{
				EFI_ACPI_3_0_FIXED_ACPI_DESCRIPTION_TABLE CONST* acpiFadt	= (EFI_ACPI_3_0_FIXED_ACPI_DESCRIPTION_TABLE CONST*)(AcpiGetFixedAcpiDescriptionTable());

				if(acpiFadt)
				{
					EFI_ACPI_DESCRIPTION_HEADER* acpiDsdt					= NULL;
	...
	__try
	{
		...
		for(UINTN i = 0; i < EfiSystemTable->NumberOfTableEntries; i++, theTable++)
		{
			...
			if(memcmp(&theTable->VendorGuid, &EfiSmbiosTableGuid, sizeof(EfiSmbiosTableGuid)) == 0)
			{
				...
				//
				SMBIOS_ENTRY_POINT_STRUCTURE *factoryEPS					= (SMBIOS_ENTRY_POINT_STRUCTURE*)(theTable->VendorTable);
				...
				CHAR8* anchorString											= (CHAR8*)(MmAllocatePool(5));
				...
				anchorString												= (CHAR8*)(MmAllocatePool(6));
			...
			{
				UINT32 address32											= (UINT32)(ArchConvertPointerToAddress(theTable->VendorTable));
		...
		{
			UINT32 address32												= (UINT32)(ArchConvertPointerToAddress(EfiSystemTable->RuntimeServices));
		...
		if(efiNode)
		{
			...
			//
			DevTreeAddProperty(efiNode, CHAR8_CONST_STRING("firmware-vendor"), EfiSystemTable->FirmwareVendor, (UINT32)((wcslen(EfiSystemTable->FirmwareVendor) + 1) * sizeof(CHAR16)), FALSE);
			...
			//
			APPLE_DEVICE_PROPERTY_PROTOCOL* devicePropertyProtocol			= NULL;
			if(!EFI_ERROR(EfiBootServices->LocateProtocol(&AppleDevicePropertyProtocolGuid, NULL, (VOID**)(&devicePropertyProtocol))))
			{
				//
				// loop read
				//
				...
				if(!EFI_ERROR(status2) && bufferLength && deviceProperties)
					DevTreeAddProperty(efiNode, CHAR8_CONST_STRING("device-properties"), deviceProperties, (UINT32)(bufferLength), TRUE);
			}
		}
