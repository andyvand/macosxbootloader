...
STATIC XML_TAG* FvpEncryptedRootPlistTag									= NULL;
STATIC UINTN FvpWrappedVolumeKeyCount										= 0;
STATIC XML_TAG* FvpWrappedVolumeKeysListTag									= NULL;
STATIC XML_TAG* FvpMasterKeyUserDictTag										= NULL;
STATIC XML_TAG* FvpRecoveryKeyUserDictTag									= NULL;

STATIC CHAR8 CONST* FvpResetIdent											= NULL;
STATIC CHAR8 CONST* FvpLoginUnlockIdent										= NULL;
STATIC CHAR8 CONST* FvpUserPassword											= NULL;
STATIC CHAR8 CONST* FvpKeyEncryptingKeyIdent								= NULL;
STATIC UINTN FvpVolumeKeyKEKLength											= 0;
STATIC UINT8 FvpVolumeKeyKEK[0x80]											= {0};

STATIC UINTN FvpVolumeKeyCount												= 0;
STATIC VOLUME_KEY_INFO* FvpVolumeKeyList									= NULL;
...
STATIC UINT64 FvpChecksum(VOID CONST* inputBuffer, UINTN bufferLength, UINT64 checksum)
{
	...
	UINT32 temp																= (UINT32)(checksum);
	UINT8 CONST* byteBuffer													= (UINT8 CONST*)(inputBuffer);
	for(UINTN i = 0; i < bufferLength; i ++, byteBuffer ++)
		temp																= crcTable[*byteBuffer ^ (UINT8)(temp)] ^ (temp >> 8);
...
STATIC BOOLEAN FvpValidateVolumeHeader(CORE_STORAGE_VOLUME_HEADER* volumeHeader)
{
	...
	return FvpChecksum(&volumeHeader->Offset08, sizeof(CORE_STORAGE_VOLUME_HEADER) - EFI_FIELD_OFFSET(CORE_STORAGE_VOLUME_HEADER, Offset08), (UINT64)(-1)) == volumeHeader->Checksum;
}
...
STATIC EFI_STATUS FvpLoadCoreStorageConfig(EFI_HANDLE coreStoragePartitionHandle)
{
    ...
    __try
	{
		...
		//
		if(EFI_ERROR(status = IoReadWholeFile(NULL, CHAR8_CONST_STRING("System\\Library\\Caches\\com.apple.corestorage\\EncryptedRoot.plist.wipekey"), &fileBuffer, &fileSize, TRUE)))
		{
            if(EFI_ERROR(status = IoReadWholeFile(NULL, CHAR8_CONST_STRING("EncryptedRoot.plist.wipekey"), &fileBuffer, &fileSize, TRUE))) {
#if defined(_MSC_VER)
                try_leave(NOTHING);
#else
                if(fileBuffer)
        ...
        if(EFI_ERROR(status = EfiBootServices->HandleProtocol(coreStoragePartitionHandle, &EfiBlockIoProtocolGuid, (VOID**)(&blockIoProtocol)))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if(fileBuffer)
        ...
        if(EFI_ERROR(status = EfiBootServices->HandleProtocol(coreStoragePartitionHandle, &EfiDiskIoProtocolGuid, (VOID**)(&diskIoProtocol)))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if(fileBuffer)
		...
		xts_decrypt(fileBuffer, (unsigned long)(fileSize), fileBuffer, initVector, &xtsContext);
		xts_done(&xtsContext);
...
STATIC BOOLEAN FvpKeyUnwrapData(VOID CONST* kekBuffer, UINTN kekLength, KEY_WRAPPED_DATA* keyWrappedData, KEY_UNWRAPPED_DATA* keyUnwrappedData)
{
	...
	keyUnwrappedData->DataLength											= (UINT32)(plaintextLength);
...
STATIC EFI_STATUS FvpDecryptVolumeKEKWithMasterKeyUser()
{
	...
	if(!FvpKeyUnwrapData(keyBuffer, sizeof(keyBuffer), (KEY_WRAPPED_DATA*)(keyWrappedKEKStructTag->DataValue), &keyUnwrappedData))
		return EFI_NOT_FOUND;
...
STATIC BOOLEAN FvpPassphaseUnwrapData(CHAR8 CONST* password, UINTN passwordLength, PASSPHASE_WRAPPED_DATA* passphaseWrappedData, PASSPHASE_UNWRAPPED_DATA* passphaseUnwrappedData)
{
	...
	passphaseUnwrappedData->DataLength										= (UINT32)(plaintextLength);
...
STATIC EFI_STATUS FvpDecryptVolumeKEKWithUserPassword(XML_TAG* cryptoUserDictTag, XML_TAG* resetUserDictTag, CHAR8 CONST* password)
{
	...
	if(!FvpPassphaseUnwrapData(password, strlen(password), (PASSPHASE_WRAPPED_DATA*)(passphraseWrappedKEKStruct->DataValue), &passphaseUnwrappedData))
		return EFI_NOT_FOUND;
	...
	for(UINTN i = 0; i < count; i ++)
	{
		...
		else if(userNameDataListTag->Type == XML_TAG_ARRAY)
		{
			...
			for(UINTN j = 0; j < nameDataCount; j ++)
			{
				...
				if(!strnicmp((CHAR8*)(userNameDataTag->DataValue), userName, userNameDataTag->DataLength))
					return cryptoUserDictTag;
			}
		}
	}
...
{
	STATIC CHAR16* inputBuffer16											= NULL;
	...
	while(!maxLength || inputLength16 < maxLength)
	{
		...
		else if(inputKey.UnicodeChar == CHAR_BACKSPACE)
		{
			...
			{
				inputLength16												-= 1;
				UINTN curColumn												= (UINTN)(EfiSystemTable->ConOut->Mode->CursorColumn);
				UINTN curRow												= (UINTN)(EfiSystemTable->ConOut->Mode->CursorRow);
				if(curColumn)
				{
					curColumn												-= 1;
				}
		...
		else if(inputKey.UnicodeChar >= ' ')
		{
			if(inputLength16 >= bufferLength)
			{
				...
				if(bufferLength < 80)
					bufferLength											= 80;
				CHAR16* newBuffer											= (CHAR16*)(MmAllocatePool(bufferLength * sizeof(CHAR16)));
	...
	if(*inputLength < inputLength16 * 3 + 2)
	{
		...
		*inputBuffer														= (CHAR8*)(MmAllocatePool(*inputLength));
	}
...
STATIC EFI_STATUS FvpUnlockCoreVolumeKey()
{
		...
		while(TRUE)
		{
			...
			if(cryptoUserDictTag)
			{
				...
				if(!selection)
				{
					...
					// unlock with it
					//
					if(!EFI_ERROR(FvpDecryptVolumeKEKWithUserPassword(cryptoUserDictTag, NULL, password)))
						break;
	...
	FvpVolumeKeyList														= (VOLUME_KEY_INFO*)(MmAllocatePool(length));
	if(!FvpVolumeKeyList)
	...
	for(UINTN i = 0; i < FvpWrappedVolumeKeyCount; i ++)
	{
		...
		KEY_WRAPPED_DATA* keyWrappedData									= (KEY_WRAPPED_DATA*)(kekWrappedVolumeKeyStructTag->DataValue);
...
EFI_STATUS FvSetupDeviceTree(UINT64* keyStorePhysicalAddress, UINTN* keyStoreSize, BOOLEAN setupDeviceTree)
{
	if(FvpLoginUnlockIdent && setupDeviceTree)
	{
		...
		{
			if(FvpResetIdent)
				DevTreeAddProperty(chosenNode, CHAR8_CONST_STRING("efilogin-reset-ident"), FvpResetIdent, (UINT32)(strlen(FvpResetIdent) + 1), FALSE);

			DevTreeAddProperty(chosenNode, CHAR8_CONST_STRING("efilogin-unlock-ident"), FvpLoginUnlockIdent, (UINT32)(strlen(FvpLoginUnlockIdent) + 1), FALSE);
			DevTreeAddProperty(chosenNode, CHAR8_CONST_STRING("efilogin-user-ident"), FvpLoginUnlockIdent, (UINT32)(strlen(FvpLoginUnlockIdent) + 1), FALSE);
		}
	}
	...
	*keyStoreSize															= totalLength;
	...
	if(totalLength && keyStorePhysicalAddress)
	{
		if(!*keyStorePhysicalAddress)
			*keyStorePhysicalAddress										= MmAllocateKernelMemory(&totalLength, NULL);
		...
		if(FvpUserPassword)
		{
			header->Type													= 2;
			header->SubType													= 0;
			header->DataLength												= (UINT32)(strlen(FvpUserPassword));
		...
		if(FvpKeyEncryptingKeyIdent)
		{
			header->Type													= 1;
			header->SubType													= 2;
			header->DataLength												= (UINT32)(FvpVolumeKeyKEKLength);
			nextOffset														+= (header->DataLength + sizeof(KEY_STORE_DATA_HEADER) + 7) & ~7;
			header->NextOffset												= nextOffset;
