...
BOOLEAN BdTrap(EXCEPTION_RECORD* exceptionRecord, struct _KEXCEPTION_FRAME* exceptionFrame, KTRAP_FRAME* trapFrame)
{
	...
	case BREAKPOINT_PROMPT:
		{
			STRING inputString;
			inputString.Length												= (UINT16)(exceptionRecord->ExceptionInformation[2]);
			inputString.Buffer												= ArchConvertAddressToPointer(exceptionRecord->ExceptionInformation[1], CHAR8*);

			STRING outputString;
			outputString.MaximumLength										= (UINT16)(trapFrame->Edi);
...
VOID BdSetContextState(DBGKD_WAIT_STATE_CHANGE64* waitStateChange, CONTEXT* contextRecord)
{
	...
	waitStateChange->ControlReport.SegCs									= (UINT16)(contextRecord->SegCs);
	waitStateChange->ControlReport.SegDs									= (UINT16)(contextRecord->SegDs);
	waitStateChange->ControlReport.SegEs									= (UINT16)(contextRecord->SegEs);
	waitStateChange->ControlReport.SegFs									= (UINT16)(contextRecord->SegFs);
...
VOID BdReadControlSpace(DBGKD_MANIPULATE_STATE64* manipulateState, STRING* additionalData, CONTEXT* contextRecord)
{
	...
	if((UINT32)(readMemory->TargetBaseAddress) + readLength <= sizeof(KPROCESSOR_STATE))
	{
		VOID* readAddress													= Add2Ptr(&BdPrcb->ProcessorState.ContextFrame.ContextFlags, (UINT32)(readMemory->TargetBaseAddress), VOID*);
		readLength															= BdMoveMemory(additionalData->Buffer, readAddress, readLength);
		additionalData->Length												= (UINT16)(readLength);
...
VOID BdWriteControlSpace(DBGKD_MANIPULATE_STATE64* manipulateState, STRING* additionalData, CONTEXT* contextRecord)
{
	...
	if((UINT32)(writeMemory->TargetBaseAddress) + writeLength <= sizeof(KPROCESSOR_STATE))
	{
		VOID* writeAddress													= Add2Ptr(&BdPrcb->ProcessorState.ContextFrame.ContextFlags, (UINT32)(writeMemory->TargetBaseAddress), VOID*);
...
VOID BdSetCommonState(UINT32 newState, CONTEXT* contextRecord, DBGKD_WAIT_STATE_CHANGE64* waitStateChange)
{
	...
	// sign extend
	//
	waitStateChange->ProgramCounter											= (UINT64)((INT32)(contextRecord->Eip));
	...
	waitStateChange->ControlReport.InstructionCount							= (UINT16)(BdMoveMemory(dstBuffer, srcBuffer, DBGKD_MAXSTREAM));
...
EFI_STATUS BdArchInitialize()
{
	...
	BdPcr																	= (KPCR*)(MmAllocatePages(AllocateMaxAddress, EfiBootServicesData, EFI_SIZE_TO_PAGES(sizeof(KPCR)), &BdPcrPhysicalAddress));
	if(!BdPcr)
	{
