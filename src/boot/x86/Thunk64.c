...
// thunk buffer
//
STATIC VOID* ArchpThunkCodeStart											= NULL;
STATIC VOID* ArchpThunkDataStart											= NULL;
STATIC UINT32 ArchpThunkDataSize											= 0;
STATIC EFI_EVENT ArchpVirtualAddressChangedEvent							= NULL;
...
// setup thunk code
//
VOID BOOTAPI ArchSetupThunkCode0(UINT64 thunkOffset, MACH_O_LOADED_INFO* loadedInfo)
{
	UINT8 i																	= 0;
	...
	else
	{
		...
		EfiRuntimeServices->GetVariable(CHAR16_STRING(L"IdlePML4"), &AppleNVRAMVariableGuid, NULL, &dataSize, &ArchpKernelIdlePML4);
	}
}
...
VOID BOOTAPI ArchSetupThunkCode1(UINT64* efiSystemTablePhysicalAddress, UINT64 thunkOffset)
{
	...
	EFI_RUNTIME_SERVICES* runtimeServicesInThunk							= (EFI_RUNTIME_SERVICES*)(ArchpThunkCodeStart);
	...
	EFI_SYSTEM_TABLE64* systemTable64										= Add2Ptr(efiMapL4, -(INTN)(efiTableSize), EFI_SYSTEM_TABLE64*);
...
EFI_STATUS BOOTAPI ArchInitialize1()
{
	...
	//
	return EfiBootServices->CreateEvent(EFI_EVENT_SIGNAL_VIRTUAL_ADDRESS_CHANGE, EFI_TPL_NOTIFY, &ArchpVirtualAddressChanged, NULL, &ArchpVirtualAddressChangedEvent);
}
