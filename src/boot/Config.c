...
// global
//
STATIC XML_TAG* CmpConfigPlistTag											= NULL;
STATIC XML_TAG* CmpNextFreeTag												= NULL;
STATIC XML_SYMBOL* CmpSymbolsHead											= NULL;
...
STATIC VOID CmpEmitString(CHAR8 CONST* theString, BOOLEAN escapeXMLChar, CHAR8** outputBuffer, UINTN* outputPosition, UINTN* outputLength)
{
	while(TRUE)
	{
		...
		if(*outputPosition + theLength >= *outputLength)
		{
			...
			CHAR8* newBuffer												= (CHAR8*)(MmAllocatePool(newLength));
			if(!newBuffer)
				return;
...
{
	XML_SYMBOL* theSymbol													= CmpFindSymbol(stringValue, NULL);
	if(!theSymbol)
	{
		theSymbol															= (XML_SYMBOL*)(MmAllocatePool(EFI_FIELD_OFFSET(XML_SYMBOL, StringBuffer) + 1 + strlen(stringValue)));
		if(!theSymbol)
			return NULL;
...
STATIC XML_TAG* CmpNewTag()
{
	if(!CmpNextFreeTag)
	{
		XML_TAG* allocatedTags												= (XML_TAG*)(MmAllocatePool(sizeof(XML_TAG) * EFI_PAGE_SIZE));
		if(!allocatedTags)
			return NULL;
		...
		for(UINT32 i = 0; i < EFI_PAGE_SIZE; i ++)
		{
			...
			allocatedTags[i].StringValue									= NULL;
			allocatedTags[i].TagValue										= NULL;
			allocatedTags[i].DataValue										= NULL;
			allocatedTags[i].NextTag										= allocatedTags + i + 1;
		}

		allocatedTags[EFI_PAGE_SIZE - 1].NextTag							= NULL;
...
{
	XML_TAG* tagList														= NULL;
	XML_TAG* prevTag														= NULL;
	INTN position															= 0;
	...
	__try
	{
		...
		tempTag->StringValue												= NULL;
		tempTag->IntegerValue												= 0;
		tempTag->DataValue													= NULL;
		tempTag->TagValue													= tagList;
		tempTag->NextTag													= NULL;
		*theTag																= tempTag;
#if defined(_MSC_VER)
	}
	__finally
...
STATIC INTN CmpParseNextTag(CHAR8* contentBuffer, XML_TAG** theTag)
{
	CHAR8* tagName															= NULL;
	*theTag																	= NULL;
	INTN length																= CmpGetNextTag(contentBuffer, &tagName, NULL);
	if(length == -1)
		return -1;
...
VOID CmFreeTag(XML_TAG* theTag)
{
	...
	theTag->StringValue														= NULL;
	theTag->IntegerValue													= 0;
	theTag->TagValue														= NULL;
	theTag->DataValue														= NULL;
...
CHAR8 CONST* CmGetStringValueForKeyAndCommandLine(CHAR8 CONST* commandLine, CHAR8 CONST* keyName, UINTN* valueLength, BOOLEAN checkConfigFile)
{
	...
	if(!retValue && checkConfigFile)
		retValue															= CmGetStringValueForKey(NULL, keyName, valueLength);
