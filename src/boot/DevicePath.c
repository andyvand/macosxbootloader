...
EFI_DEVICE_PATH_PROTOCOL* DevPathDuplicate(EFI_DEVICE_PATH_PROTOCOL* devicePath)
{
	...
	EFI_DEVICE_PATH_PROTOCOL* newDevicePath									= (EFI_DEVICE_PATH_PROTOCOL*)(MmAllocatePool(size));
	if(!newDevicePath)
		return NULL;
...
EFI_DEVICE_PATH_PROTOCOL* DevPathAppendFilePath(EFI_DEVICE_PATH_PROTOCOL* devicePath, CHAR16 CONST* fileName)
{
	...
	EFI_DEVICE_PATH_PROTOCOL* filePath										= (EFI_DEVICE_PATH_PROTOCOL*)(MmAllocatePool(size + devicePathSize + SIZE_OF_FILEPATH_DEVICE_PATH));
	if(!filePath)
		return NULL;
...
EFI_DEVICE_PATH_PROTOCOL* DevPathAppendLastComponent(EFI_DEVICE_PATH_PROTOCOL* devicePath, CHAR8 CONST* fileName, BOOLEAN replaceLastComponent)
{
	...
	EFI_DEVICE_PATH_PROTOCOL* newDevicePath									= (EFI_DEVICE_PATH_PROTOCOL*)(MmAllocatePool(length));
	if(!newDevicePath)
		return NULL;

	EFI_DEVICE_PATH_PROTOCOL* dstDevicePath									= newDevicePath;
	EFI_DEVICE_PATH_PROTOCOL* srcDevicePath									= devicePath;
	FILEPATH_DEVICE_PATH* srcFilePath										= NULL;
	FILEPATH_DEVICE_PATH* dstFilePath										= NULL;
...
{
	FILEPATH_DEVICE_PATH* startFilePath										= NULL;
	FILEPATH_DEVICE_PATH* endFilePath										= NULL;
	UINTN totalLength														= 0;
	while(!EfiIsDevicePathEnd(devicePath))
	{
	...
	CHAR16* allocatedPathName												= NULL;
	UINTN pathLength														= 0;
	if(startFilePath != endFilePath)
	{
		allocatedPathName													= (CHAR16*)(MmAllocatePool(totalLength));
...
EFI_DEVICE_PATH_PROTOCOL* DevPathGetDevicePathProtocol(EFI_HANDLE theHandle)
{
	...
	EfiBootServices->HandleProtocol(theHandle, &EfiDevicePathProtocolGuid, (VOID**)(&retValue));
	return retValue;
}
...
EFI_HANDLE DevPathGetPartitionHandleByNumber(EFI_DEVICE_PATH_PROTOCOL* referencePath, UINT32 number)
{
	...
	if(EFI_ERROR(EfiBootServices->LocateHandleBuffer(ByProtocol, &EfiBlockIoProtocolGuid, NULL, &count, &handleArray)))
		return NULL;

	EFI_HANDLE retValue														= NULL;
