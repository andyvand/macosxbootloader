...
//
STATIC VOID* Bd1394pControllerRegisterBase									= NULL;

//
// global data
//
STATIC DBG1394_GLOBAL_DATA* Bd1394pGlobalData								= NULL;
...
{
	UINT32 temp																= 0;
	UINT32* quadlet															= (UINT32*)(inputBuffer);
...
STATIC BOOLEAN Bd1394pInitializeController(UINT32 channel)
{
	...
	//
	UINT32 temp																= ARCH_READ_REGISTER_UINT32((UINT32*)(Bd1394pControllerRegisterBase));
	...
	temp																	= (0xffffff & (UINT32)(Bd1394pGetPhysicalAddress(&Bd1394pGlobalData->DebugConfigInfo))) | 0x1e000000;
	...
	//
	ARCH_WRITE_REGISTER_UINT32(Add2Ptr(Bd1394pControllerRegisterBase, 0x34, UINT32*), (UINT32)(Bd1394pGetPhysicalAddress(&Bd1394pGlobalData->DebugConfigRom)));
...
STATIC VOID Bd1394pCheckRom(DBG1394_GLOBAL_DATA* globalData)
{
	...
	// get our rom address
	//
	UINT32 dbgRom															= (UINT32)(Bd1394pGetPhysicalAddress(&globalData->DebugConfigRom));
...
UINT32 Bd1394ReceivePacket(UINT32 packetType, STRING* messageHeader, STRING* messageData, UINT32* dataLength)
{
	...
	if(packetType == PACKET_TYPE_KD_POLL_BREAKIN)
	{
		...
		// breakin?
		//
		if((UINT8)(Bd1394pGlobalData->ReceiveBuffer.ReceiveBuffer.PacketHeader.PacketLeader) != BREAKIN_PACKET_BYTE)
			return KDP_PACKET_TIMEOUT;
	...
	for( ; TRUE ; Bd1394pSendControlPacket(PACKET_TYPE_KD_RESEND, 0))
	{
		...
		if(ret != KDP_PACKET_RECEIVED)
		{
			...
			// check breakin
			//
			if((UINT8)(packetHeader.PacketLeader) == BREAKIN_PACKET_BYTE)
			{
...
VOID Bd1394SendPacket(UINT32 packetType, STRING* messageHeader, STRING* messageData)
{
	...
	while(TRUE)
	{
		...
		{
			DBGKD_WAIT_STATE_CHANGE64* stateChange							= (DBGKD_WAIT_STATE_CHANGE64*)((VOID*)(messageHeader->Buffer));
			DBGKD_DEBUG_IO* debugIo											= (DBGKD_DEBUG_IO*)((VOID*)(messageHeader->Buffer));
			DBGKD_FILE_IO* fileIo											= (DBGKD_FILE_IO*)((VOID*)(messageHeader->Buffer));
	...
	__try
	{
        ...
        if(!loaderOptions || !loaderOptions[0]) {
            ...
            if(EFI_ERROR(status))
            {
                ...
                Bd1394pGlobalData                                                = NULL;
                Bd1394pDataPhysicalAddress                                        = 0;
                Bd1394pControllerRegisterBase                                    = NULL;
            }
		...
		channel																= (UINT32)(atoi(channleParam + 9));
		...
		if(connectWait)
			EfiBootServices->Stall(1000 * 1000 * (UINT32)(atoi(connectWait + 13)));
		...
		// parse location
		//
		UINTN segment														= (UINTN)(-1);
		UINTN bus															= (UINTN)(-1);
		UINTN device														= (UINTN)(-1);
		UINTN func															= (UINTN)(-1);
		...
		EfiBootServices->DisconnectController(controllerHandle, NULL, NULL);
		status																= EfiBootServices->HandleProtocol(controllerHandle, &firewireProtocolGuid, &protocol);
		CsPrintf(CHAR8_CONST_STRING("handle protocol %08x, %08x\n"), status, protocol);
