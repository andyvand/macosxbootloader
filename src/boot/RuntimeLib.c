...
{
	UINT8* byteDstBuffer													= (UINT8*)(dstBuffer);
	UINT8 CONST* byteSrcBuffer												= (UINT8 CONST*)(srcBuffer);
...
{
	UINT8 CONST* byteBuffer1												= (UINT8 CONST*)(buffer1);
	UINT8 CONST* byteBuffer2												= (UINT8 CONST*)(buffer2);
	for(UINTN i = 0; i < bufferLength; i ++)
	{
...
STATIC UINTN _value_to_mhex_string(CHAR8* dstBuffer, UINT64 value, UINTN flags, UINTN width)
{
	...
	if(width > 0 && (UINTN)(tempString - tempBuffer) > width)
...
STATIC UINTN _value_to_string(CHAR8* dstBuffer, INT64 value, UINTN flags, UINTN width)
{
	...
	do
	{
		UINTN remainder														= (UINT64)(value) % 10;
		value																= (INT64)((UINT64)(value) / 10);

		*tempString															= (CHAR8)(remainder + '0');
		tempString															+= 1;
		count																+= 1;
		numberCount															+= 1;
...
UINTN vsnprintf(CHAR8* dstBuffer, UINTN bufferLength, CHAR8 CONST* formatString, VA_LIST marker)
{
	...
	for(index = 0; *formatString && index < bufferLength - 1; formatString ++)
	{
		...
		else
		{
			...
			case 'S':
				{
					...
					{
						dstBuffer[index]									= (CHAR8)(*unicodeString);
						index												+= 1;
					}
			...
			case 'c':
				dstBuffer[index]											= (CHAR8)(VA_ARG(marker, UINTN));
				index														+= 1;
				break;
