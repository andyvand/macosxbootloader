...
// global
//
STATIC DEVICE_TREE_NODE* DevTreepRootNode									= NULL;
STATIC DEVICE_TREE_NODE* DevTreepNextFreeNode								= NULL;
STATIC DEVICE_TREE_NODE* DevTreepAllocatedNodes								= NULL;
STATIC DEVICE_TREE_PROPERTY* DevTreepNextFreeProperty						= NULL;
STATIC DEVICE_TREE_PROPERTY* DevTreepAllocatedProperties					= NULL;
...
{
	if(!DevTreeAddChild(NULL, CHAR8_CONST_STRING("/")))
...
STATIC DEVICE_TREE_PROPERTY* DevTreepGetProperty()
{
	if(!DevTreepNextFreeProperty)
	{
		...
		DEVICE_TREE_PROPERTY* allocatedProperty								= (DEVICE_TREE_PROPERTY*)(propertiesBuffer);
...
STATIC DEVICE_TREE_NODE* DevTreepGetNode()
{
	if(!DevTreepNextFreeNode)
	{
		...
		DEVICE_TREE_NODE* allocatedNode										= (DEVICE_TREE_NODE*)(nodesBuffer);
...
CHAR8 CONST* DevTreeGetName(DEVICE_TREE_NODE* theNode)
{
	for(DEVICE_TREE_PROPERTY* theProperty	= theNode->Properties; theProperty; theProperty = theProperty->Next)
	{
		if(!strcmp(theProperty->Name, CHAR8_CONST_STRING("name")))
			return (CHAR8*)(theProperty->Value);
	}
...
DEVICE_TREE_NODE* DevTreeAddChild(DEVICE_TREE_NODE* parentNode, CHAR8 CONST* childName)
{
	...
	EFI_STATUS status														= DevTreeAddProperty(theNode, CHAR8_CONST_STRING("name"), childName, (UINT32)(strlen(childName) + 1) * sizeof(CHAR8), TRUE);
	if(!EFI_ERROR(status))
		return theNode;
...
EFI_STATUS DevTreeAddProperty(DEVICE_TREE_NODE* theNode, CHAR8 CONST* propertyName, VOID CONST* propertyValue, UINT32 valueLength, BOOLEAN allocAndCopy)
{
	...
	{
		theProperty->Value													= (VOID*)(propertyValue);
...
VOID DevTreeFinalize()
{
	...
	DevTreepNextFreeNode													= NULL;
	DevTreepAllocatedNodes													= NULL;
	DevTreepRootNode														= NULL;
	...
	DevTreepNextFreeProperty												= NULL;
	DevTreepAllocatedProperties												= NULL;
...
STATIC UINT8* DevTreepFlattenNode(DEVICE_TREE_NODE* theNode, UINT8* flattenBuffer)
{
	...
	DEVICE_TREE_NODE_FLATTEN* flatNode										= (DEVICE_TREE_NODE_FLATTEN*)((VOID*)(flattenBuffer));
	flattenBuffer															+= sizeof(DEVICE_TREE_NODE_FLATTEN);
	for(DEVICE_TREE_PROPERTY* theProperty	= theNode->Properties; theProperty; theProperty = theProperty->Next)
	{
		DEVICE_TREE_PROPERTY_FLATTEN* flatProperty							= (DEVICE_TREE_PROPERTY_FLATTEN*)((VOID*)(flattenBuffer));
...
EFI_STATUS DevTreeFlatten(VOID** flattenBuffer, UINT32* bufferLength)
{
	...
	if(flattenBuffer)
	{
		if(totalLength)
		{
			...
			DevTreepFlattenNode(DevTreepRootNode, (UINT8*)(deviceTreeBuffer));
		}
