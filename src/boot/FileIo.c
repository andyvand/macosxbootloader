...
// booting from net
//
STATIC EFI_LOAD_FILE_PROTOCOL* IopLoadFileProtocol							= NULL;
STATIC EFI_FILE_HANDLE IopRootFile											= NULL;
...
STATIC EFI_STATUS IopFindBootDevice(EFI_HANDLE* bootDeviceHandle, EFI_DEVICE_PATH_PROTOCOL** bootFilePath)
{
	...
	__try
	{
		...
		//
		UINTN handleCount													= 0;
		EfiBootServices->LocateHandleBuffer(ByProtocol, &EfiSimpleFileSystemProtocolGuid, NULL, &handleCount, &handleArray);
		...
		for (UINTN i = 0; i < handleCount; i ++)
		{
			...
			{
				EFI_FILE_HANDLE checkFile									= NULL;
	...
	__finally
	{
		...
		for (UINTN i = 0; i < ARRAYSIZE(foundHandle); i ++)
		{
			...
			FILEPATH_DEVICE_PATH* filePath									= (FILEPATH_DEVICE_PATH*)(bootFilePathBuffer);
...
STATIC EFI_STATUS IopDetectRoot(EFI_HANDLE deviceHandle, EFI_DEVICE_PATH_PROTOCOL* bootFilePath, BOOLEAN allowBootDirectory)
{
	...
	EFI_FILE_HANDLE realRootFile											= NULL;
	CHAR8* bootFullPath														= NULL;
	CHAR16* bootFullPath16													= NULL;
	IopRootFile																= NULL;
	IopLoadFileProtocol														= NULL;
	...
	__try
	{
        ...
        if (EFI_ERROR(status = EfiBootServices->HandleProtocol(deviceHandle, &EfiSimpleFileSystemProtocolGuid, (VOID**)(&fileSystemProtocol)))) {
#if defined(_MSC_VER)
            try_leave(status = EfiBootServices->HandleProtocol(deviceHandle, &EfiLoadFileProtocolGuid, (VOID**)(&IopLoadFileProtocol)));
#else
            status = EfiBootServices->HandleProtocol(deviceHandle, &EfiLoadFileProtocolGuid, (VOID**)(&IopLoadFileProtocol));
            ...
            if (IopRootFile && EFI_ERROR(status))
                IopRootFile->Close(IopRootFile), IopRootFile = NULL;
		...
		if (!EFI_ERROR(realRootFile->Open(realRootFile, &kernelFile, CHAR16_STRING(L"EncryptedRoot.plist.wipekey"), EFI_FILE_MODE_READ, 0)))
			...
			realRootFile->Close(realRootFile), realRootFile = NULL;
#if defined(_MSC_VER)
	}
	__finally
	{
...
STATIC EFI_STATUS IopLoadBooterWithRootUUID(EFI_DEVICE_PATH_PROTOCOL* bootFilePath, EFI_HANDLE theHandle, CHAR8 CONST* rootUUID, EFI_HANDLE* imageHandle)
{
	...
	*imageHandle															= NULL;
    ...
    __try
	{
        ...
        if (EFI_ERROR(status = EfiBootServices->HandleProtocol(theHandle, &EfiSimpleFileSystemProtocolGuid, (VOID**)(&fileSystemProtocol)))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if (recoveryFilePath)
		...
		// load boot file
		//
        if (EFI_ERROR(status = IoReadWholeFile(bootFilePath, CHAR8_CONST_STRING("Library\\Preferences\\SystemConfiguration\\com.apple.Boot.plist"), &fileBuffer, NULL, TRUE))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            if (recoveryFilePath)
		...
		// read root UUID
		//
		CHAR8 CONST* theRootUUID											= CmGetStringValueForKey(rootTag, CHAR8_CONST_STRING("Root UUID"), NULL);
...
EFI_STATUS IoLoadBooterWithRootUUID(EFI_DEVICE_PATH_PROTOCOL* bootFilePath, CHAR8 CONST* rootUUID, EFI_HANDLE* imageHandle)
{
	...
	__try
	{
		...
		UINTN handleCount													= 0;
		EfiBootServices->LocateHandleBuffer(ByProtocol, &EfiBlockIoProtocolGuid, NULL, &handleCount, &handleArray);
...
EFI_STATUS IoOpenFile(CHAR8 CONST* filePathName, EFI_DEVICE_PATH_PROTOCOL* filePath, IO_FILE_HANDLE* fileHandle, UINTN openMode)
{
	...
	__try
	{
		...
		if (IopLoadFileProtocol)
		{
			...
			FILEPATH_DEVICE_PATH* fileDevicePath							= (FILEPATH_DEVICE_PATH*)(MmAllocatePool(filePathNodeSize + END_DEVICE_PATH_LENGTH));
            if (!fileDevicePath) {
#if defined(_MSC_VER)
                try_leave(status = EFI_OUT_OF_RESOURCES);
#else
...
EFI_STATUS IoSetFilePosition(IO_FILE_HANDLE* fileHandle, UINT64 filePosition)
{
	...
	fileHandle->FileOffset													= (UINTN)(filePosition);
...
EFI_STATUS IoGetFileSize(IO_FILE_HANDLE* fileHandle, UINT64* fileSize)
{
	*fileSize																= 0;
	...
	if (fileHandle->EfiLoadFileProtocol)
	{
		...
		{
			EFI_STATUS status												= fileHandle->EfiLoadFileProtocol->LoadFile(fileHandle->EfiLoadFileProtocol, fileHandle->EfiFilePath, FALSE, &fileHandle->FileSize, NULL);
...
EFI_STATUS IoGetFileInfo(IO_FILE_HANDLE* fileHandle, EFI_FILE_INFO** fileInfo)
{
	*fileInfo																= NULL;
	...
	EFI_FILE_INFO* infoBuffer												= (EFI_FILE_INFO*)(MmAllocatePool(infoSize));
	...
	if (status == EFI_BUFFER_TOO_SMALL)
	{
		...
		infoBuffer															= (EFI_FILE_INFO*)(MmAllocatePool(infoSize));
    ...
    __try
	{
		...
		*readLength															= 0;
		...
		if (fileHandle->EfiLoadFileProtocol)
        {
			...
			fileHandle->FileSize											= (UINTN)(fileSize);

			if (!fileHandle->FileBuffer)
			{
				fileHandle->FileBuffer										= (UINT8*)(MmAllocatePool((UINTN)(fileSize)));
				...
				UINTN read_length											= (UINTN)(fileSize);
...
#if defined(_MSC_VER)
                    try_leave(MmFreePool(fileHandle->FileBuffer); fileHandle->FileBuffer = NULL; status = EFI_DEVICE_ERROR);
#else
                    MmFreePool(fileHandle->FileBuffer);
                    fileHandle->FileBuffer = NULL;
		...
		UINT8* curBuffer													= (UINT8*)(readBuffer);
		while(bufferSize)
		{
...
EFI_STATUS IoReadWholeFile(EFI_DEVICE_PATH_PROTOCOL* bootFilePath, CHAR8 CONST* fileName, CHAR8** fileBuffer, UINTN* fileSize, BOOLEAN asTextFile)
{
	...
	*fileBuffer																= NULL;
	...
	__try
	{
		...
		// allocate buffer
		//
		UINTN totalSize														= (UINTN)(localFileSize) + (asTextFile ? sizeof(CHAR8) : 0);
		*fileBuffer															= (CHAR8*)(MmAllocatePool(totalSize));
        ...
        if (EFI_ERROR(status = IoReadFile(&fileHandle, *fileBuffer, (UINTN)(localFileSize), &readLength, FALSE))) {
#if defined(_MSC_VER)
            try_leave(NOTHING);
#else
            IoCloseFile(&fileHandle);
