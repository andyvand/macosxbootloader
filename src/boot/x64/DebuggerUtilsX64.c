	...
	case BREAKPOINT_PRINT:
		{
			...
			if(BdDebuggerNotPresent)
				trapFrame->Rax												= (UINT64)((INT64)(STATUS_DEVICE_NOT_CONNECTED));
			else
				trapFrame->Rax												= (UINT64)((UINT64)(BdPrintString(&printString) ? STATUS_BREAKPOINT : STATUS_SUCCESS));
	...
	case BREAKPOINT_PROMPT:
		{
			...
			STRING outputString;
			outputString.MaximumLength										= (UINT16)(trapFrame->R9);
...
#if !defined(_MSC_VER)
VOID __bzero(VOID *s, UINT64 n)
{
    __builtin_bzero(s, n);
}

VOID *_alloca(UINT64 n)
{
